...
source("R/global_vars_and_libs.R")
```
...
```{r kenlm-training-as-file-onepct-make}
tar_make()
```
...
Now, modeling actually has a cutoff value, that is, a lot of monograms are not even allowed to be in the model. Aditionally, it is just plain normal that the test and devtest sets have words that are unks.
...
```{targets unkify-devtest-target}
tar_target(unkified_devtest,
tar_read(clean_devtest_onepct) %>% 
    n_gramify_unked_all(text_dt = .,
