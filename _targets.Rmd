...
In the kenLM way, we might preserve the strategy all the way to splits. It is the way we unkify, model and evaluate a corpus where things change.

At the end of this branch of the pipeline, we will have targets that compare corpuses lowercased to normal, different cutoff values, and different end model
sizes.


So, first we build two full versions of the raw corpus both lower and upper case:

```{targets kenlm-pipeline-target}

tarchetypes::tar_map(
    values=tibble(to_lower=c(TRUE,FALSE)),
    tar_target(
        preclean_lowercase,
        preclean_files(files = list.files(en_data_dir, pattern = '*.txt'),
                       to_lower = to_lower)
    ),
    tar_target(sentencified_lowercase,
               sentencify(preclean_lowercase),
                format = "file")
) %>% list()
```
